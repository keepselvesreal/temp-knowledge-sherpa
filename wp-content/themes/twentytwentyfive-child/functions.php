 ...
 * 카테고리 사이드바 HTML 생성
 */
function twentytwentyfive_child_render_category_sidebar($parent_categories) {
    $html = '';
...
/**
 * 동적으로 사이드바 콘텐츠 생성 (하위 호환성)
 */
function twentytwentyfive_child_render_dynamic_sidebar() {
    global $sidebar_parent_categories;
    return twentytwentyfive_child_render_category_sidebar($sidebar_parent_categories);
}
...
/**
 * Dynamic Block: Category Navigation Render Callback
 */
function twentytwentyfive_child_render_category_navigation($attributes) {
    $parent_categories = $attributes['parentCategories'] ?? array(11, 19, 17);
    return twentytwentyfive_child_render_category_sidebar($parent_categories);
}

/**
 * Register Dynamic Blocks
 */
function twentytwentyfive_child_register_dynamic_blocks() {
    // Category Navigation Block
    wp_register_script(
        'twentytwentyfive-child-category-navigation-editor',
        get_stylesheet_directory_uri() . '/blocks/category-navigation/index.js',
        array('wp-blocks', 'wp-element', 'wp-block-editor', 'wp-components', 'wp-data'),
        filemtime(get_stylesheet_directory() . '/blocks/category-navigation/index.js')
    );

    register_block_type(
        get_stylesheet_directory() . '/blocks/category-navigation',
        array(
            'editor_script' => 'twentytwentyfive-child-category-navigation-editor',
            'render_callback' => 'twentytwentyfive_child_render_category_navigation',
        )
    );
